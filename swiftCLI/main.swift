...
print("Welcome to the swiftCLI project generator")
...
print("---------------------------------------------------------------------")
print("swiftCLI will now generate a project with the following parameters:")
...
do {
    let fileManager = FileManager.default
    let temporaryDirectoryPath = destination + "/swiftcli_temp"
    let gitClonePath = "\(temporaryDirectoryPath)/swiftCLI"
    ...
    performCommand(description: "Making a local clone of the swiftCLI repo") {
        let repositoryURL = arguments.repositoryURL ?? URL(string: "https://github.com/krishpranav/swiftCLI.git")!
